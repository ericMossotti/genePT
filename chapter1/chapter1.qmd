...
```{bash}
source renv/profiles/chapter1/renv/python/virtualenvs/renv-python-3.12/bin/activate
```
...
renv::activate(profile = "chapter1")
```
...
pbmc.data <- Read10X(
    data.dir = "pbmc3k_filtered_gene_bc_matrices/filtered_gene_bc_matrices/hg19")
...
ggsave(filename = "./output/images/pbmc3k_umap.jpg", 
       height = 7, 
       width = 12, 
       plot = plot, 
       quality = 50)
...
#| label: "save.times"
#| include: false
write.csv(x = t(as.data.frame(all_times)), 
          file = "./output/timings/pbmc3k_tutorial_times.csv")
```
